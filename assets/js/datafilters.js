const dataFilters = {
	contributionsData: {
		columns: [{
			...
			name: "DonorId",
			type: "number",
			filterFunction: n => n > 0 && n <= 207 //currently the max ID in the donors master table
		}, {
			name: "PooledFundId",
			type: "number",
			filterFunction: n => n > 0 && n <= 251 //currently the max ID in the funds master table
		}, {
			name: "PledgeAmt",
			type: "number",
	...
	allocationsData: {
		columns: [{
			...
			name: "ProjList",
			type: t => typeof t === "number" || (typeof t === "string" && t.includes("##")), //the value is either a number or, if a string, it has a separator "##"
			filterFunction: null
		}, {
			name: "NumbofProj",
			type: "number",
