...
function createCountryProfileByPartnerAndSector(
	container,
	lists,
	colors,
	tooltipDiv,
	...
	const selectionChartDiv = chartsDiv
		.append("div")
		.attr("class", classPrefix + "selectionChartDiv");

	const selectionChartDivTitle = selectionChartDiv
		.append("div")
		.attr("class", classPrefix + "selectionChartDivTitle");

	const selectionChartDivContent = selectionChartDiv
		.append("div")
	...
	function draw(originalData, resetYear, firstTime) {
		...
		function reselectCards(cardsContainer) {
			const cards = cardsContainer.selectAll(
				`.${classPrefix}partnersCard`
			);
			const title = cardsContainer.select(
				...
				if (!d.clicked) {
					...
					cards
						.style("background-color", e =>
							e.sector === d.sector ? lightUnBlue : null
						)
						.each(
							(d, i, n) =>
...
function drawTopFigures(
	...
	partnerFigures
		...
		.style("color", d =>
			d.fund.size > 1 ? null : colors[Array.from(d.fund)[0]]
		);
...
function createTopFiguresDiv(container, colors, lists) {
	...
	const partnersFiguresDivWrapper = container
		.append("div")
		.attr("class", classPrefix + "partnersFiguresDivWrapper");

	const partnerFiguresDiv = partnersFiguresDivWrapper
		.append("div")
	...
	const sectorsDivTitle = sectorsDiv
		.append("div")
...
function drawSelectionChart(
	...
	const partnersCardIconDiv = partnersCardEnter
		.append("div")
...
function drawTable(
	...
	const filteredData = JSON.parse(
		JSON.stringify(!sector ? data : data.filter(e => e.sector === sector))
	);
	...
	function sortRows(sortType) {
		...
		filteredData.sort((a, b) =>
			sortType === "name"
				...
				: sortType === "type"
				? lists.clustersList[a.sector].localeCompare(
						lists.clustersList[b.sector]
				  )
				: b.value - a.value
		);
		rowDiv
...
function processData(originalData, lists) {
	...
	if (chartState.selectedFund !== "cbpf")
		originalData.cerf.forEach(row =>
			processRow(row, data.cerfData, "cerf")
		);
	if (chartState.selectedFund !== "cerf")
		originalData.cbpf.forEach(row =>
			processRow(row, data.cbpfData, "cbpf")
		);
	...
	function processRow(row, target, fundType) {
		if (chartState.selectedYearCountryProfile.includes(row.year)) {
			row.values.forEach(innerRow => {
				...
				const foundSector = data.dataAggregated.find(
					e => e.sector === innerRow.sector
				);
				if (foundSector) {
				...
				const foundPartner = data.topFigures.partnerFigures.find(
