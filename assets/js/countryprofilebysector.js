...
function createCountryProfileBySector(
	container,
	lists,
	colors,
	tooltipDiv,
	...
	const yScale = d3.scaleBand().paddingInner(0.5).paddingOuter(0.5);
...
function drawTopFigures(data, container, colors, syncedTransition, tooltipDiv) {
	...
	container
		.select(`.${classPrefix}sectorsValue`)
		.transition(syncedTransition)
		.call(applyColors, colors)
		.tween("html", (_, i, n) => {
			const interpolator = d3.interpolateRound(
...
function drawStackedChart({
	...
	const maxHeight = Math.min(
		svgHeight,
		padding[0] + padding[2] + data.length * barHeight
	);
	...
	let title = svg.selectAll(`.${classPrefix}title`).data([true]);
	...
	let xAxisGroup = svg.selectAll(`.${classPrefix}xAxisGroup`).data([true]);
	...
	let yAxisGroup = svg.selectAll(`.${classPrefix}yAxisGroup`).data([true]);
	...
	const barsGroupsEnter = barsGroups
		.enter()
		.append("g")
	...
	let labels = svg
		.selectAll("." + classPrefix + "labelsBySector")
		.data(filteredData, d => d.sector);

	const labelsExit = labels
		.exit()
		.transition()
		.duration(duration)
		.style("opacity", 0)
	...
	let clusterIcons = svg
		.selectAll("." + classPrefix + "clusterIcons")
		.data(filteredData, d => d.sector);

	const clusterIconsExit = clusterIcons
		.exit()
		.transition()
		.duration(duration)
		.style("opacity", 0)
	...
	const clusterIconsEnter = clusterIcons
		.enter()
		.append("image")
		...
		.attr(
			"x",
			padding[3] - clusterIconPadding - clusterIconSize - yAxis.tickSize()
		)
		.attr(
			"y",
			d => yScale(d.sector) - (clusterIconSize - yScale.bandwidth()) / 2
		)
	...
	let barsTooltipRectangles = svg
		.selectAll("." + classPrefix + "barsTooltipRectangles")
		.data(filteredData, d => d.sector);

	const barsTooltipRectanglesExit = barsTooltipRectangles.exit().remove();

	const barsTooltipRectanglesEnter = barsTooltipRectangles
		.enter()
		.append("rect")
	...
	barsTooltipRectangles = barsTooltipRectanglesEnter.merge(
		barsTooltipRectangles
	);
	...
	function customAxis(group) {
		...
		sel.selectAll(".tick text")
			...
			.call(
				wrapTextTwoLines,
				padding[3] - yAxis.tickPadding() - axisPadding
			);
		if (sel !== group)
			group
				.selectAll(".tick text")
	...
	barsTooltipRectangles
		.on("mouseover", (event, d) =>
			mouseoverBars(event, d, tooltipDiv, container, colors, lists)
		)
		.on("mouseout", () => mouseOut(tooltipDiv));
...
function createTopFiguresDiv(container, colors, lists) {
	...
	const sectorsDivWrapper = container
		.append("div")
		.attr("class", classPrefix + "sectorsDivWrapper");

	const sectorsDiv = sectorsDivWrapper
		.append("div")
		.attr("class", classPrefix + "sectorsDiv");

	const sectorsValue = sectorsDiv
		.append("span")
		.attr("class", classPrefix + "sectorsValue")
		.html("0")
		.call(applyColors, colors);

	const sectorsText = sectorsDiv
		.append("span")
		...
		["cerf", "cbpf"]
			...
			.forEach(fund => {
				...
				fundDiv
					...
					.style(
						"color",
						d3.color(colors[fund]).darker(darkerValueText)
					)
...
function processData(originalData, lists) {
	...
	originalData.forEach(row => {
		if (
			chartState.selectedYearCountryProfile.includes(row.year) &&
			...
			if (
				...
				if (row.partnersCerf)
					row.partnersCerf
						.toString()
						.split(separator)
						.forEach(e => data.topFigures.partners.add(e));
				if (row.projectsCbpf)
					row.projectsCbpf
						.toString()
						.split(separator)
				...
				if (row[chartState.selectedFund])
					data.topFigures.sectors.add(row.sector);
				if (row[`projects${capitalize(chartState.selectedFund)}`])
					row[`projects${capitalize(chartState.selectedFund)}`]
						.toString()
						.split(separator)
						.forEach(e => data.topFigures.projects.add(e));
				if (row[`partners${capitalize(chartState.selectedFund)}`])
					row[`partners${capitalize(chartState.selectedFund)}`]
						.toString()
						.split(separator)
						.forEach(e => data.topFigures.partners.add(e));
			}
			...
			copiedRow.total =
				chartState.selectedFund === "total" ? copiedRow.total : 0;
			copiedRow.cerf =
			...
			if (chartState.selectedYearCountryProfile.length === 1) {
				...
				const foundSector = data.stack.find(
					e => e.sector === copiedRow.sector
				);
				if (foundSector) {
...
function disableFunds(data, fundButtons) {
	["cerf", "cbpf"].forEach(fund => {
		const filteredData = data.filter(e =>
			chartState.selectedYearCountryProfile.includes(e.year)
		);
...
function disableYears(data, yearsButtons) {
	if (
		...
		yearsButtons
			.style("opacity", d =>
				thisYearSet.has(d) ? 1 : fadeOpacityFundButton
			)
